...
export default function Edit({ attributes, setAttributes }) {
	...
	const prepareAvatarBorder = (border) => {
		...
		if (avatarBorderTop === "") {
			...
			if (avatarBorderTop !== "") {
				avatarBorderTop = `${avatarBorderTop} ${
		...
		if (avatarBorderRight === "") {
			...
			if (avatarBorderRight !== "") {
				avatarBorderRight = `${avatarBorderRight} ${
					border.style ? border.style : "solid"
				} ${border.color ? border.color : ""}`;
			}
		}
		...
		if (avatarBorderBottom === "") {
			...
			if (avatarBorderBottom !== "") {
				avatarBorderBottom = `${avatarBorderBottom} ${
					border.style ? border.style : "solid"
				} ${border.color ? border.color : ""}`;
			}
		}
		...
		if (avatarBorderLeft === "") {
			...
			if (avatarBorderLeft !== "") {
				avatarBorderLeft = `${avatarBorderLeft} ${
					border.style ? border.style : "solid"
				} ${border.color ? border.color : ""}`;
			}
		}
	...
	useEffect(() => {
		...
		// Card
		r.style.setProperty(
			"--pd-list-card-background-color",
			attributes.cardBackgroundColor
		);
		r.style.setProperty("--pd-list-card-border", `${attributes.cardBorder}px`);
		r.style.setProperty(
			"--pd-list-card-border-radius",
			`${attributes.cardBorderRadius}px`
		);

		// Title
		r.style.setProperty("--pd-list-title-color", attributes.titleColor);
		r.style.setProperty(
			"--pd-list-title-font-size",
		...
		r.style.setProperty(
			"--pd-list-meta-key-font-size",
			`${attributes.metaKeyFontSize}px`
		);

		r.style.setProperty(
			"--pd-list-meta-value-color",
			attributes.metaValueColor
		);
		r.style.setProperty(
			"--pd-list-meta-value-font-size",
			`${attributes.metaValueFontSize}px`
		);

		// Category
		r.style.setProperty(
			"--pd-list-category-label-color",
			attributes.categoryLabelColor
		);
		r.style.setProperty(
			"--pd-list-category-label-font-size",
			`${attributes.categoryLabelFontSize}px`
		);

		r.style.setProperty(
			"--pd-list-category-value-color",
			attributes.categoryValueColor
		);
		r.style.setProperty(
			"--pd-list-category-value-font-size",
			`${attributes.categoryValueFontSize}px`
		);

		// Excerpt
		r.style.setProperty("--pd-list-excerpt-color", attributes.excerptColor);
		r.style.setProperty(
			"--pd-list-excerpt-font-size",
			`${attributes.excerptFontSize}px`
		);
		r.style.setProperty("--pd-list-read-more-color", attributes.readMoreColor);
		r.style.setProperty(
			"--pd-list-read-more-font-size",
			`${attributes.readMoreFontSize}px`
		);
		...
		r.style.setProperty(
			"--pd-list-avatar-border-radius",
			`${attributes.avatarBorderRadius}%`
		);
		r.style.setProperty(
			"--pd-list-author-name-color",
			attributes.authorNameColor
		);
		r.style.setProperty(
			"--pd-list-author-name-font-size",
			`${attributes.authorNameFontSize}px`
		);
	}, []);
	...
	return (
		<div {...blockProps}>
			<InspectorControls key={"settings"}>
				...
				<Panel>
					<PanelBody title={__("Query", "post-designer")} initialOpen={false}>
						...
						/>
						<Divider></Divider>
						<label className="pd-gb-label">{__("Term", "post-designer")}</label>
						{termsTemplate()}
				...
				<Panel>
					<PanelBody
						title={__("Pagination", "post-designer")}
						initialOpen={false}
					>
						<NumberControl
						...
						<ToggleControl
							...
							help={__(
								"On only if you want to display all posts together",
								"post-designer"
							)}
				...
				{/* style controls */}
				<Panel>
					<PanelBody initialOpen={false} title={__("Card", "post-designer")}>
						<BaseControl label={__("Background Color", "post-designer")}>
							<ColorPalette
					...
					<PanelBody title={__("Meta", "post-designer")} initialOpen={false}>
						...
						{attributes.showMeta ? (
							<div>
								...
								<BaseControl label={__("Value Color", "post-designer")}>
									<ColorPalette
				...
				<Panel>
					<PanelBody
						title={__("Category", "post-designer")}
						initialOpen={false}
					>
						<ToggleControl
							...
							<div>
								...
								<BorderBoxControl
									...
									onChange={(value) => {
										...
										r.style.setProperty(
											"--pd-list-avatar-border-top",
											avatarBorder.top
										);
										r.style.setProperty(
											"--pd-list-avatar-border-left",
											avatarBorder.left
										);
										r.style.setProperty(
											"--pd-list-avatar-border-right",
											avatarBorder.right
										);
										r.style.setProperty(
											"--pd-list-avatar-border-bottom",
											avatarBorder.bottom
										);
									}}
			...
			{loading ? (
				...
				<>
					<div
						className={`pd-card-row ${
							posts.length ? `pd-${columnPerRow}-col` : ""
						}`}
					>
						{renderPostList()}
					</div>
